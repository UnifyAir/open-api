...
impl DeserResponse for CreateMbsPolicyResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			201 => {
				...
				Ok((status, CreateMbsPolicyResponse::Status201 {
					body,
					location,
				}))
			}
			308 => {
				...
				Ok((status, CreateMbsPolicyResponse::Status308 {
					body,
					location,
			...
			400 => {
				...
				Ok((status, CreateMbsPolicyResponse::Status400(body)))
			}
			401 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, CreateMbsPolicyResponse::Status401(body)))
			}
			403 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, CreateMbsPolicyResponse::Status403(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, CreateMbsPolicyResponse::Status404(body)))
			}
			411 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, CreateMbsPolicyResponse::Status411(body)))
			}
			413 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, CreateMbsPolicyResponse::Status413(body)))
			}
			415 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, CreateMbsPolicyResponse::Status415(body)))
			}
			429 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, CreateMbsPolicyResponse::Status429(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, CreateMbsPolicyResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, CreateMbsPolicyResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, CreateMbsPolicyResponse::Statusdefault))
			}
		}
	}
}

/// MbsPoliciesCollection
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait MbsPoliciesCollection {
	/// Request the creation of a new MBS Policy Association..
	///
	/// CreateMbsPolicy - POST /npcf-mbspolicycontrol/v1/mbs-policies
	async fn create_mbs_policy(
		&self,
		method: Method,
		host: Host,
		cookies: CookieJar,
		body: models::MbsPolicyCtxtData,
	) -> Result<CreateMbsPolicyResponse, String>;
}

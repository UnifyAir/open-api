...
#[repr(u16)]
#[serde(untagged)]
pub enum DeleteEventsSubscResponse {
...
impl DeserResponse for DeleteEventsSubscResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			204 => {
				...
				Ok((status, DeleteEventsSubscResponse::Status204))
			}
			307 => {
				...
				Ok((status, DeleteEventsSubscResponse::Status307 {
					body,
					location,
			...
			308 => {
				...
				Ok((status, DeleteEventsSubscResponse::Status308 {
					body,
					location,
					param_3gpp_sbi_target_nf_id,
				}))
			...
			_ => {
				...
				Ok((status, DeleteEventsSubscResponse::Statusdefault))
			}
		}
	}
}
...
#[repr(u16)]
#[serde(untagged)]
pub enum UpdateEventsSubscResponse {
	/// The modification of the Events Subscription resource is confirmed its
	/// representation is returned.
	Status200(models::EventsSubscPutData) = 200,
	/// The creation of the Events Subscription resource is confirmed and its
	/// representation is returned.
	Status201 {
		body: models::EventsSubscPutData,
		location: String,
	} = 201,
	/// The modification of the Events Subscription subresource is confirmed
	/// without returning additional data.
	Status204 = 204,
	/// Temporary Redirect
	Status307 {
...
impl DeserResponse for UpdateEventsSubscResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		let status = resp.status();
		...
		match status.as_u16() {
			200 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				let body: models::EventsSubscPutData = serde_json::from_str(&data)?;

				Ok((status, UpdateEventsSubscResponse::Status200(body)))
			}
			201 => {
				let location: String = resp
					.headers()
				...
				Ok((status, UpdateEventsSubscResponse::Status201 {
					body,
					location,
				}))
			}
			204 => {
				...
				Ok((status, UpdateEventsSubscResponse::Status204))
			}
			307 => {
				let location: String = resp
					.headers()
				...
				Ok((status, UpdateEventsSubscResponse::Status307 {
					body,
					location,
					param_3gpp_sbi_target_nf_id,
				}))
			...
			308 => {
				...
				Ok((status, UpdateEventsSubscResponse::Status308 {
					body,
					location,
					param_3gpp_sbi_target_nf_id,
				}))
			...
			_ => {
				...
				Ok((status, UpdateEventsSubscResponse::Statusdefault))
			}
		}
	}
}

/// EventsSubscriptionDocument
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait EventsSubscriptionDocument {
	/// deletes the Events Subscription subresource.
	///
	/// DeleteEventsSubsc - DELETE
	/// /npcf-policyauthorization/v1/app-sessions/{appSessionId}/
	/// events-subscription
	async fn delete_events_subsc(
		&self,
		method: Method,
		host: Host,
		cookies: CookieJar,
		path_params: models::DeleteEventsSubscPathParams,
	) -> Result<DeleteEventsSubscResponse, String>;
	...
	) -> Result<UpdateEventsSubscResponse, String>;
}

...
// TODO: Perf check. Add it to mustache and regenerate the code.
// TODO: All headers extraction code, move to util, and use the util function
// for code generation
impl TryFrom<IntoHeaderValue<RetryAfter>> for HeaderValue {
	type Error = String;

	fn try_from(value: IntoHeaderValue<RetryAfter>) -> Result<Self, Self::Error> {
		let value: reqwest::header::HeaderValue = value.0.try_into().unwrap();
		let value: HeaderValue = unsafe { HeaderValue::from_maybe_shared_unchecked(value) };
		Ok(value)
	}
}

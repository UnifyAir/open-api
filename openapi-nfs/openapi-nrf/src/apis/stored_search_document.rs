...
impl DeserResponse for RetrieveStoredSearchResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				Ok((status, RetrieveStoredSearchResponse::Status200 {
					body,
					cache_control,
					etag,
					content_encoding,
			...
			307 => {
				...
				Ok((status, RetrieveStoredSearchResponse::Status307 {
					body,
					location,
				}))
			}
			308 => {
				...
				Ok((status, RetrieveStoredSearchResponse::Status308 {
					body,
					location,
				}))
			}
...
pub trait StoredSearchDocument {
	...
	) -> Result<RetrieveStoredSearchResponse, String>;
}

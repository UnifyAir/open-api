...
impl DeserResponse for GetNssaiResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::Nssai = serde_json::from_str(&data)?;

				Ok((status, GetNssaiResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetNssaiResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetNssaiResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetNssaiResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetNssaiResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetNssaiResponse::Statusdefault))
			}
		}
	}
}
...
pub trait SliceSelectionSubscriptionDataRetrieval {
	...
	) -> Result<GetNssaiResponse, String>;
}

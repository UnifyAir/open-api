...
impl DeserResponse for GetEcrDataResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::EnhancedCoverageRestrictionData = serde_json::from_str(&data)?;

				Ok((status, GetEcrDataResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetEcrDataResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetEcrDataResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetEcrDataResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetEcrDataResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetEcrDataResponse::Statusdefault))
			}
		}
	}
}

/// EnhancedCoverageRestrictionDataRetrieval
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait EnhancedCoverageRestrictionDataRetrieval {
	...
	) -> Result<GetEcrDataResponse, String>;
}

...
impl DeserResponse for GetLcsPrivacyDataResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::LcsPrivacyData = serde_json::from_str(&data)?;

				Ok((status, GetLcsPrivacyDataResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetLcsPrivacyDataResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetLcsPrivacyDataResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetLcsPrivacyDataResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetLcsPrivacyDataResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetLcsPrivacyDataResponse::Statusdefault))
			}
		}
	}
}

/// LcsPrivacyDataRetrieval
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait LcsPrivacyDataRetrieval {
	...
	) -> Result<GetLcsPrivacyDataResponse, String>;
}

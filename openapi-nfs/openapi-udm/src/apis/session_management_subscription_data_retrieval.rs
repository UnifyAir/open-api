...
impl DeserResponse for GetSmDataResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::SmSubsData = serde_json::from_str(&data)?;

				Ok((status, GetSmDataResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetSmDataResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSmDataResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSmDataResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSmDataResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetSmDataResponse::Statusdefault))
			}
		}
	}
}
...
pub trait SessionManagementSubscriptionDataRetrieval {
	...
	) -> Result<GetSmDataResponse, String>;
}

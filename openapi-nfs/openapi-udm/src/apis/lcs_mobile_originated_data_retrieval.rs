...
impl DeserResponse for GetLcsMoDataResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::LcsMoData = serde_json::from_str(&data)?;

				Ok((status, GetLcsMoDataResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetLcsMoDataResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetLcsMoDataResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetLcsMoDataResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetLcsMoDataResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetLcsMoDataResponse::Statusdefault))
			}
		}
	}
}

/// LcsMobileOriginatedDataRetrieval
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait LcsMobileOriginatedDataRetrieval {
	...
	) -> Result<GetLcsMoDataResponse, String>;
}

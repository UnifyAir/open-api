...
impl DeserResponse for GetSharedDataResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: Vec<models::SharedData> = serde_json::from_str(&data)?;

				Ok((status, GetSharedDataResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetSharedDataResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSharedDataResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSharedDataResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSharedDataResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetSharedDataResponse::Statusdefault))
			}
		}
	}
}

/// RetrievalOfSharedData
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait RetrievalOfSharedData {
	...
	) -> Result<GetSharedDataResponse, String>;
}

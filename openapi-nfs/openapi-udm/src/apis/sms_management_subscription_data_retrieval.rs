...
impl DeserResponse for GetSmsMngtDataResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::SmsManagementSubscriptionData = serde_json::from_str(&data)?;

				Ok((status, GetSmsMngtDataResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetSmsMngtDataResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSmsMngtDataResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSmsMngtDataResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSmsMngtDataResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetSmsMngtDataResponse::Statusdefault))
			}
		}
	}
}

/// SmsManagementSubscriptionDataRetrieval
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait SmsManagementSubscriptionDataRetrieval {
	...
	) -> Result<GetSmsMngtDataResponse, String>;
}

...
#[repr(u16)]
#[serde(untagged)]
pub enum Create5GMbsGroupResponse {
...
impl DeserResponse for Create5GMbsGroupResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			201 => {
				...
				Ok((status, Create5GMbsGroupResponse::Status201))
			}
			400 => {
				let data = resp.text().await?;
			...
			_ => {
				...
				Ok((status, Create5GMbsGroupResponse::Statusdefault))
			}
		}
	}
}
...
pub trait Class5GmbsGroupCreation {
	...
	) -> Result<Create5GMbsGroupResponse, String>;
}

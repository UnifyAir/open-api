...
impl DeserResponse for GetSmfSelDataResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::SmfSelectionSubscriptionData = serde_json::from_str(&data)?;

				Ok((status, GetSmfSelDataResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetSmfSelDataResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSmfSelDataResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSmfSelDataResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetSmfSelDataResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetSmfSelDataResponse::Statusdefault))
			}
		}
	}
}
...
pub trait SmfSelectionSubscriptionDataRetrieval {
	...
	) -> Result<GetSmfSelDataResponse, String>;
}

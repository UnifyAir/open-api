...
impl DeserResponse for GetDataSetsResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::SubscriptionDataSets = serde_json::from_str(&data)?;

				Ok((status, GetDataSetsResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetDataSetsResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetDataSetsResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetDataSetsResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetDataSetsResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetDataSetsResponse::Statusdefault))
			}
		}
	}
}

/// RetrievalOfMultipleDataSets
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait RetrievalOfMultipleDataSets {
	...
	) -> Result<GetDataSetsResponse, String>;
}

...
impl DeserResponse for GetAmDataResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::AccessAndMobilitySubscriptionData = serde_json::from_str(&data)?;

				Ok((status, GetAmDataResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetAmDataResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetAmDataResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetAmDataResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetAmDataResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetAmDataResponse::Statusdefault))
			}
		}
	}
}
...
pub trait AccessAndMobilitySubscriptionDataRetrieval {
	...
	) -> Result<GetAmDataResponse, String>;
}

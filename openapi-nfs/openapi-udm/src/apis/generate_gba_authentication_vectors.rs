...
#[repr(u16)]
#[serde(untagged)]
pub enum GenerateGbaAvResponse {
...
impl DeserResponse for GenerateGbaAvResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::GbaAuthenticationInfoResult = serde_json::from_str(&data)?;

				Ok((status, GenerateGbaAvResponse::Status200(body)))
			}
			400 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
			...
			_ => {
				...
				Ok((status, GenerateGbaAvResponse::Statusdefault))
			}
		}
	}
}

/// GenerateGbaAuthenticationVectors
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait GenerateGbaAuthenticationVectors {
	/// Generate authentication data for the UE in GBA domain.
	///
	/// GenerateGbaAv - POST
	/// /nudm-ueau/v1/{supi}/gba-security-information/generate-av
	async fn generate_gba_av(
		&self,
		method: Method,
		host: Host,
		cookies: CookieJar,
	...
	) -> Result<GenerateGbaAvResponse, String>;
}

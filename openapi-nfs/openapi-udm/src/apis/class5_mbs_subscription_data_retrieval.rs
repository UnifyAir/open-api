...
impl DeserResponse for GetMbsDataResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::MbsSubscriptionData = serde_json::from_str(&data)?;

				Ok((status, GetMbsDataResponse::Status200 {
					body,
					cache_control,
					etag,
					last_modified,
			...
			400 => {
				...
				Ok((status, GetMbsDataResponse::Status400(body)))
			}
			404 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetMbsDataResponse::Status404(body)))
			}
			500 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetMbsDataResponse::Status500(body)))
			}
			503 => {
				let data = resp.text().await?;
				// Deserialize body only when dataType is present and no headers
				...
				Ok((status, GetMbsDataResponse::Status503(body)))
			}
			_ => {
				let data = resp.text().await?;

				Ok((status, GetMbsDataResponse::Statusdefault))
			}
		}
	}
}

/// Class5MbsSubscriptionDataRetrieval
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait Class5MbsSubscriptionDataRetrieval {
	...
	) -> Result<GetMbsDataResponse, String>;
}

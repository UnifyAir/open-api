...
/// The Nullable type. Represents a value which may be specified as null on an
/// API. Note that this is distinct from a value that is optional and not
/// present!
///
/// Nullable implements many of the same methods as the Option type (map,
/// unwrap, etc).
...
impl<T> Nullable<T> {
	...
	///
	/// ```
	/// # use openapi_nssf::types::Nullable;
	///
	...
	/// # Examples
	///
	/// Convert an `Nullable<`[`String`]`>` into a `Nullable<`[`usize`]`>`,
	/// preserving the original. The [`map`] method takes the `self` argument
	/// by value, consuming the original, so this technique uses `as_ref` to
	/// first take a `Nullable` to a reference to the value inside the
	/// original.
	///
	...
	/// # Panics
	///
	/// Panics if the value is a [`Nullable::Null`] with a custom panic message
	/// provided by `msg`.
	///
	...
	/// Moves the value `v` out of the `Nullable<T>` if it is
	/// `Nullable::Present(v)`.
	///
	...
	/// Maps a `Nullable<T>` to `Nullable<U>` by applying a function to a
	/// contained value.
	///
	/// # Examples
	///
	/// Convert a `Nullable<`[`String`]`>` into a `Nullable<`[`usize`]`>`,
	...
	/// Transforms the `Nullable<T>` into a [`Result<T, E>`], mapping
	/// `Nullable::Present(v)` to [`Ok(v)`] and `Nullable::Null` to
	/// [`Err(err)`][Err].
	///
	...
	/// ```
	#[inline]
	pub fn ok_or_else<E, F: FnOnce() -> E>(
		self,
		err: F,
	) -> Result<T, E> {
		match self {
	...
	/// Returns `Nullable::Null` if the Nullable is `Nullable::Null`, otherwise
	/// returns `optb`.
	///
	/// # Examples
	///
	...
	/// ```
	#[inline]
	pub fn and_then<U, F: FnOnce(T) -> Nullable<U>>(
		self,
		f: F,
	) -> Nullable<U> {
		match self {
	...
	/// ```
	#[inline]
	pub fn or_else<F: FnOnce() -> Nullable<T>>(
		self,
		f: F,
	) -> Nullable<T> {
		match self {
	...
	/// Takes the value out of the Nullable, leaving a `Nullable::Null` in its
	/// place.
	///
	/// # Examples
	///
...
impl<'a, T: Clone> Nullable<&'a T> {
	/// Maps an `Nullable<&T>` to an `Nullable<T>` by cloning the contents of
	/// the Nullable.
	///
	/// # Examples
	///
...
impl<T: Default> Nullable<T> {
	/// Returns the contained value or a default
	///
	/// Consumes the `self` argument then, if `Nullable::Present`, returns the
	/// contained value, otherwise if `Nullable::Null`, returns the default
	/// value for that type.
	///
	/// # Examples
	///
	...
	T: serde::de::DeserializeOwned,
{
		...
		D: Deserializer<'de>,
	{
		...
		// serde_json::Value, which fails if the value is missing, or gives
		// serde_json::Value::Null if the value is present. If that succeeds as null,
		// we can easily return a Null. If that succeeds as some value, we deserialize
		// that value and return a Present. If that errors, we return the error.
...
macro_rules! impl_val_range {
	($t:tt) => {
		impl ValidateRange<$t> for Nullable<$t> {
			fn greater_than(
				&self,
				max: $t,
			) -> Option<bool> {
				match self {
					Nullable::Present(value) => Some(value > &max),
					Nullable::Null => None, // Assuming `None` is considered valid
				}
			}
			...
			fn less_than(
				...
				match self {
					Nullable::Present(value) => Some(value < &min),
					Nullable::Null => None, // Assuming `None` is considered valid
				}
			}
		}
		// impl ValidateRange<$t> for Option<Nullable<$t>> {
		//     fn greater_than(&self, max: $t) -> Option<bool> {
		//         self.map_or(None, |r: Nullable<$t>| r.greater_than(max))
		//     }

		//     fn less_than(&self, min: $t) -> Option<bool> {
		//         self.map_or(None, |r: Nullable<$t>| r.less_than(min))
		//     }
		// }

		// impl ValidateRange<$t> for Option<Nullable<$t>> {
		//     fn greater_than(&self, max: $t) -> Option<bool> {
		//         self.flatten().map(|r: Nullable<$t>| r.greater_than(max))
		//     }

		//     fn less_than(&self, min: $t) -> Option<bool> {
		//         self.flatten().map(|r: Nullable<$t>| r.less_than(min))
		//     }
		// }
	};
}
...
macro_rules! validate_type_with_len {
    ($type_:ty, $($generic:ident),*$(,)*) => {
        impl<$($generic),*> ValidateLength<u64> for Nullable<$type_> {
            fn length(&self) -> Option<u64> {
                match self {
                    Nullable::Present(value) => Some(value.len() as u64),
                    Nullable::Null => None, // Assuming `None` is considered valid
                }
            }
        }

...
#[repr(u16)]
#[serde(untagged)]
pub enum CreateUpuProtectionResponse {
	/// UpuSecurityInfo
	Status200(models::UpuSecurityInfo) = 200,
	/// Temporary Redirect
	Status307 {
...
impl DeserResponse for CreateUpuProtectionResponse {
	async fn deserialize(resp: reqwest::Response) -> Result<(reqwest::StatusCode, Self), ReqError> {
		...
		match status.as_u16() {
			200 => {
				...
				let body: models::UpuSecurityInfo = serde_json::from_str(&data)?;

				Ok((status, CreateUpuProtectionResponse::Status200(body)))
			}
			307 => {
				...
				Ok((status, CreateUpuProtectionResponse::Status307 {
					body,
					location,
			...
			308 => {
				...
				Ok((status, CreateUpuProtectionResponse::Status308 {
					body,
					location,
					param_3gpp_sbi_target_nf_id,
				}))
			...
			503 => {
				...
				Ok((status, CreateUpuProtectionResponse::Status503(body)))
			}
...
/// UeParametersUpdateProtection
#[async_trait]
#[allow(clippy::ptr_arg)]
pub trait UeParametersUpdateProtection {
	/// CreateUpuProtection - POST /nausf-upuprotection/v1/{supi}/ue-upu
	async fn create_upu_protection(
		&self,
		method: Method,
		host: Host,
		cookies: CookieJar,
		...
		body: models::UpuInfo,
	) -> Result<CreateUpuProtectionResponse, String>;
}
